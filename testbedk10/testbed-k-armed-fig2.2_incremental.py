...
def plot_and_save(
        ...
        for time_step in range(time_steps):
            ...
            for i, eps in enumerate(eps_greedies):
                ...
                if np.random.rand() < eps:
                    ...
                    # greedy selection
                    sel_action = np.argmax(Q_reward[i])
                
                R_t = np.random.normal(Q_reward[i, sel_action], 1)

                total_reward[i] += R_t
                Q_reward[i, sel_action] += 1. / (time_step + 1) *\
                    (R_t - Q_reward[i, sel_action])
    ...
    plt.savefig(f'./testbedk10/Fig. 2.2 incremental {eps_greedies}.png')
